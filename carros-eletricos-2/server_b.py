...
mqtt_port = 1883
mqtt_topic = constants.TOPICO_BATERIA.format(server="server_b")
...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe(constants.TOPICO_RESERVA.format(server="server_b"))
...
def on_message(client, userdata, msg):
    try:
        ...
        if msg.topic == mqtt_topic:
            ...
            logger.info(f"Server B received battery update: {vehicle_id}, {battery_level}%")
            
            if battery_level < 30:
                threading.Thread(target=handle_low_battery, args=(vehicle_id, data.get("current_city"), data.get("end_city"))).start()
        
        elif msg.topic == constants.TOPICO_RESERVA.format(server="server_b"):
                ...
                if point["location"] == current_city:
                    if point["reserved"] < point["capacity"]:
                        ...
                        prepare_response = requests.post(
                            constants.SERVERS["company_b"]["url"] + "/api/prepare",
            ...
            for r in reservations:
                if r["company"] == "company_b":
                    ...
                    try:
                        ...
                        logger.error(f"Server B: Error aborting reservation: {e}")
                ...
                for point in charging_points:
                    if point["id"] == r["point_id"] and "position" in r:
                        if vehicle_id in point["queue"]:
                            point["queue"].remove(vehicle_id)
                            point["reserved"] += 1
                logger.info(f"Server B: Committed local reservation for {r['city']}, point {r['point_id']}")
            elif r["url"]:
                try:
                    requests.post(
                        f"{r['url']}/api/commit",
                    ...
                    logger.error(f"Server B: Error committing reservation: {e}")
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=constants.SERVIDOR_B)