...
# Simulated database for Company D's charging points (Pernambuco)
charging_points = [
    {
        "id": "PE1", 
        "location": "Recife", 
...
mqtt_port = 1883
mqtt_topic = constants.TOPICO_BATERIA.format(server="server_d")
...
def handle_low_battery(vehicle_id, current_city, end_city):
    try:
        logger.info(f"Server D handling low battery for {vehicle_id} in {current_city}")
        
        local_points = [p for p in charging_points if p["location"] in ["Recife", "Caruaru"]]
        ...
        error_msg = f"Server D error: {str(e)}"
        logger.error(error_msg)
        mqtt_client.publish(
            constants.TOPICO_RESPOSTA.format(vehicle_id=vehicle_id),
            json.dumps({
...
def on_connect(client, userdata, flags, rc):
    logger.info(f"Server D connected to MQTT broker with code {rc}")
    client.subscribe(mqtt_topic)
    client.subscribe(constants.TOPICO_RESERVA.format(server="server_d"))
...
def on_message(client, userdata, msg):
    try:
        ...
        if msg.topic == mqtt_topic:
            ...
            logger.info(f"Server D received battery update: {vehicle_id}, {battery_level}%")
            
            if battery_level < 30:
                threading.Thread(target=handle_low_battery, args=(vehicle_id, data.get("current_city"), data.get("end_city"))).start()
        
        elif msg.topic == constants.TOPICO_RESERVA.format(server="server_d"):
...
def get_charging_points():
    logger.info("Server D: Returning charging points")
    return jsonify({"company_d": charging_points})
...
def plan_route_for_vehicle(vehicle_id, start, end):
    logger.info(f"Server D: Planning route for vehicle {vehicle_id} from {start} to {end}")
    try:
        path = nx.shortest_path(G, start, end, weight="weight")
        logger.info(f"Server D: Shortest path: {path}")
            ...
            for point in charging_points:
                if point["location"] == current_city:
                    if point["reserved"] < point["capacity"]:
                        ...
                        reserved = True
                        logger.info(f"Server D: Prepared local reservation for {current_city}, point {point['id']}")
                        break
                    else:
                        prepare_response = requests.post(
                            constants.SERVERS["company_d"]["url"] + "/api/prepare",
            ...
            if not reserved:
                for company, url in servers.items():
                    try:
                        ...
                        logger.error(f"Server D: Error contacting {company}: {e}")
                        all_prepared = False
            ...
            for r in reservations:
                if r["company"] == "company_d":
                    ...
                    try:
                        ...
                        logger.error(f"Server D: Error aborting reservation: {e}")
                ...
                for point in charging_points:
                    if point["id"] == r["point_id"] and "position" in r:
                        if vehicle_id in point["queue"]:
                            point["queue"].remove(vehicle_id)
                            point["reserved"] += 1
                logger.info(f"Server D: Committed local reservation for {r['city']}, point {r['point_id']}")
            elif r["url"]:
                try:
                    requests.post(
                        f"{r['url']}/api/commit",
                    ...
                    logger.error(f"Server D: Error committing reservation: {e}")
        ...
        logger.error(f"Server D: Route planning error: {e}")
        mqtt_client.publish(
            constants.TOPICO_RESPOSTA.format(vehicle_id=vehicle_id),
            json.dumps({
                "status": "ERROR",
...
def plan_route():
    ...
    if not start or not end or not vehicle_id:
        logger.error("Server D: Missing start, end, or vehicle_id")
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=constants.SERVIDOR_D)